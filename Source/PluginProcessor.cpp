...
SimplePluginAudioProcessor::SimplePluginAudioProcessor()
    ...
    : 
#endif
    parameters(*this, nullptr,juce::Identifier("LowpassandHighpassPlugin"), {std::make_unique<juce::AudioParameterFloat>("highpasscutoff_frequency", "HighPass Cutoff Frequency", juce::NormalisableRange{20.f,20000.f,0.1f,0.2f, false}, 0.f),
        std::make_unique<juce::AudioParameterFloat>("lowpasscutoff_frequency", "LowPass Cutoff Frequency", juce::NormalisableRange{20.f,20000.f,0.1f,0.2f,false},20000.f) })
{
        highpassParameter = parameters.getRawParameterValue("highpasscutoff_frequency");
        lowpassParameter = parameters.getRawParameterValue("lowpasscutoff_frequency");
...
void SimplePluginAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    highPassfilter.setSamplingRate(sampleRate);
    lowPassfilter.setSamplingRate(sampleRate);
...
void SimplePluginAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    const auto highCutoffFreq = highpassParameter->load();
    const auto lowCutoffFreq = lowpassParameter->load();
    highPassfilter.setCutoffFrequency(highCutoffFreq);
    lowPassfilter.setCutoffFrequency(lowCutoffFreq);
    highPassfilter.processBlock(buffer, midiMessages);
